...
if (!$flight) {
    header('Location: ../simulations.php?error=flight_not_found');
    exit;
}
...
if (empty($telemetry_url)) {
    header('Location: ../simulations.php?error=no_telemetry_data');
    exit;
}
?>
<!DOCTYPE html>
...
<div id="app" style="display:none">
  ...
  <div id="topbar">
    <a class="brand" href="#">
      ...
      <div class="brand-tag" style="background:var(--s)">REPLAY</div>
    </a>
    <div class="vsep"></div>
    <div class="tsp"></div>
    <button class="nbtn sm danger" id="exit-sim-btn" onclick="closeReplay()" title="Close Replay Window">�xa� Close</button>
  ...
  <div id="lpanel">
    ...
    <div class="card card-sm" style="pointer-events:none;opacity:0.85;">
      ...
      <div class="fmode-grid">
        <button class="fmode-btn" data-mode="stabilized"><span class="fm-icon">�a️</span>Stabilized</button>
        <button class="fmode-btn" data-mode="angle"><span class="fm-icon">�x�</span>Angle</button>
        <button class="fmode-btn" data-mode="acro"><span class="fm-icon">�x}�</span>Acro</button>
        <button class="fmode-btn" data-mode="althold"><span class="fm-icon">�x</span>Alt Hold</button>
        <button class="fmode-btn" data-mode="gpshold"><span class="fm-icon">�x�</span>GPS Hold</button>
        <button class="fmode-btn" data-mode="rth"><span class="fm-icon">�x��</span>RTH</button>
      </div>
    </div>

    <!-- Environment (Active, allow user to change sky/terrain) -->
    <div class="card card-sm">
    ...
    <!-- Drone Profile (Read Only) -->
    <div class="card card-sm" style="pointer-events:none;opacity:0.85;">
      <div class="card-title"><span class="ct-dot"></span>DRONE PROFILE (RECORDED)</div>
      <div style="display:flex;flex-direction:column;gap:8px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
          <div style="font-size:12px;color:var(--txt2);">Label</div><div id="drone-profile-label" style="font-size:12px;font-weight:700;">--</div>
          <div style="font-size:12px;color:var(--txt2);">Mass</div><div id="drone-mass-val" style="font-size:12px;font-weight:700;">--</div>
          <div style="font-size:12px;color:var(--txt2);">Battery</div><div id="drone-batt-val" style="font-size:12px;font-weight:700;">--</div>
          <div style="font-size:12px;color:var(--txt2);">Max RPM</div><div id="drone-maxrpm-val" style="font-size:12px;font-weight:700;">--</div>
        </div>
      </div>
    </div>
  ...
  <div id="bottombar" style="grid-column:2/3;background:var(--surf);border-top:1px solid var(--n3);padding:10px 20px;display:flex;align-items:center;gap:18px;height:80px;">
    ...
    <!-- Time display -->
    <div style="font-family:var(--fh);font-size:13px;font-weight:600;color:var(--txt2);min-width:110px;text-align:center;" id="timeline-time-bottom">
      00:00 / 00:00
    </div>
    
    <!-- Seek/Scrub Slider -->
    <div style="flex:1;display:flex;align-items:center;">
      <input type="range" min="0" max="100" value="0" id="replay-timeline" class="accent-range" style="width: 100%; height: 6px; cursor: pointer;">
    </div>
...
function clearMotorFailures() {
  ...
  if (typeof UI !== 'undefined') UI.toast('�S& Motors restored');
}
