...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['save_profile']))       { $name=$_POST['full_name']??$name; $profile_saved=true; }
...
.sidebar-sep{font-size:10px;font-weight:600;letter-spacing:.1em;color:var(--text3);padding:12px 14px 2px;text-transform:uppercase;}
.sidebar-spacer{flex:1;}
.sidebar-bottom{padding-top:14px;border-top:1px solid var(--border);display:flex;flex-direction:column;gap:4px;}
...
.user-action-btn.logout:hover{background:rgba(224,85,85,.12);color:var(--red);}
...
<aside class="sidebar">
  <div class="sidebar-logo">
    <svg width="28" height="28" viewBox="0 0 32 32" fill="none"><circle cx="16" cy="16" r="4" fill="#10256D"/><rect x="6" y="14" width="8" height="4" rx="2" fill="#10256D" opacity=".7"/><rect x="18" y="14" width="8" height="4" rx="2" fill="#10256D" opacity=".7"/><rect x="14" y="6" width="4" height="8" rx="2" fill="#10256D" opacity=".7"/><rect x="14" y="18" width="4" height="8" rx="2" fill="#10256D" opacity=".7"/><circle cx="7" cy="7" r="3" fill="#EE9346"/><circle cx="25" cy="7" r="3" fill="#EE9346"/><circle cx="7" cy="25" r="3" fill="#EE9346"/><circle cx="25" cy="25" r="3" fill="#EE9346"/></svg>
    <span class="sidebar-logo-text">DRONESIM</span>
  </div>
  <a class="nav-item" href="dashboard.php"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>Dashboard</a>
  <a class="nav-item" href="simulations.php"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5,3 19,12 5,21"/></svg>Simulations</a>
  <a class="nav-item" href="new-session.php"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>New Session</a>
  <div class="sidebar-sep">Account</div>
  <a class="nav-item" href="team.php"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>Team</a>
  <a class="nav-item" href="admin.php"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Admin</a>
  <div class="sidebar-spacer"></div>
  <div class="sidebar-bottom">
    <div class="user-chip">
      <div class="avatar"><?= $initials ?></div>
      <div style="flex:1;min-width:0;"><div class="user-name"><?= $name ?></div><div class="user-role"><?= $plan ?> plan</div></div>
      <div class="user-actions">
        <a href="settings.php" class="user-action-btn" title="Settings" style="color:var(--secondary);">
        ...
        <a href="auth/logout.php" class="user-action-btn logout" title="Logout"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg></a>
      </div>
    </div>
  </div>
</aside>
...
<main class="main">
  <header class="topbar">
    ...
    <div class="topbar-right">
      ...
      <button class="icon-btn" aria-label="Notifications"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg></button>
    </div>
  </header>
  <div class="content">
    ...
    <div class="settings-card">
      ...
      <div class="profile-head">
        ...
        <div><div class="profile-name"><?= $name ?></div><div class="profile-email"><?= $email ?></div></div>
      </div>
      <?php if ($profile_saved): ?><div class="toast"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20,6 9,17 4,12"/></svg>Profile saved.</div><?php endif; ?>
      <form method="POST" action="?tab=profile">
        <div class="form-grid-2" style="margin-bottom:20px;">
          <div class="form-group"><label class="form-label">Full Name</label><input class="form-input" type="text" name="full_name" value="<?= $name ?>" placeholder="Your full name" required/></div>
          <div class="form-group"><label class="form-label">Email</label><input class="form-input" type="email" name="email" value="<?= $email ?>" placeholder="you@example.com" required/></div>
          <div class="form-group full"><label class="form-label">Time Zone</label>
            <select class="form-select" name="timezone"><?php
      ...
      <?php if ($password_saved): ?><div class="toast"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20,6 9,17 4,12"/></svg>Password updated.</div><?php endif; ?>
      <form method="POST" action="?tab=profile">
        <div class="form-grid-2" style="margin-bottom:20px;">
          <div class="form-group"><label class="form-label">Current Password</label><input class="form-input" type="password" name="current_password" placeholder="Current password"/></div>
          <div class="form-group"><label class="form-label">New Password</label><input class="form-input" type="password" id="new_password" name="new_password" placeholder="Min 8 characters" minlength="8"/><div class="pw-strength-bar"><div class="pw-strength-fill" id="pwFill"></div></div></div>
        </div>
      ...
      <form method="POST" action="?tab=notifications">
        <div class="notif-list"><?php
          ...
          <div class="notif-row">
            ...
            <div class="toggle-wrap"><input class="toggle-input" type="checkbox" id="n_<?= $k ?>" name="<?= $k ?>" <?= $notif[$k]?'checked':'' ?>/><label class="toggle-track" for="n_<?= $k ?>"></label></div>
          </div>
        <?php endforeach; ?></div>
        <div style="margin-top:24px;"><button class="btn-primary" type="submit" name="save_notifications">Save Preferences</button></div>
      </form>
    </div>
    <?php endif; ?>
  </div>
...
(function(){
  ...
  t.addEventListener('click',function(){var n=h.getAttribute('data-theme')==='dark'?'light':'dark';h.setAttribute('data-theme',n);localStorage.setItem('sb_theme',n);s();});
})();
(function(){
  var p=document.getElementById('new_password'),f=document.getElementById('pwFill');
  if(!p||!f)return;
  p.addEventListener('input',function(){var v=p.value,s=0;if(v.length>=8)s++;if(v.length>=12)s++;if(/[A-Z]/.test(v))s++;if(/[0-9]/.test(v))s++;if(/[^A-Za-z0-9]/.test(v))s++;f.style.width=Math.round(s/5*100)+'%';f.style.background=s<=1?'#e05555':s<=3?'#EE9346':'#28c840';});
})();
</script>
</body>
</html>